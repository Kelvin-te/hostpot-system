                    ...
                    <form method="post" action="{{ route('users.update', $user->id) }}" class="mt-6 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __("Phone is the hotspot login identity and cannot be changed here.") }}</p>
                            </div>

                            <div>
                                <div>
                                    <x-input-label for="phone" :value="__('Phone number')" class="mt-4"></x-input-label>
                                    <x-text-input id="phone" type="text" class="mt-1 block w-full bg-gray-100" value="{{ $user->detail->phone }}" disabled></x-text-input>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __("Change the active package for this user.") }}</p>
                            </div>
                                ...
                                <div>
                                    ...
                                    <select id="package_id" name="package_id" class="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" required>
                                        ...
                                        @foreach($packages as $package)
                                            <option value="{{ $package->id }}" {{ old('package_id', $currentPackage?->id) == $package->id ? 'selected' : '' }}>{{ $package->name }} - {{ config('app.currency') }} {{ $package->price }}</option>
                                        @endforeach
                                    </select>
